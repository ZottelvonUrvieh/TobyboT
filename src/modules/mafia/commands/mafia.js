...
let addGame = async function (msg, args, cmd, settings) {
    let games = await cmd.bot.dbManager.getTableRowsByKey(table, { owner_id: msg.author.id });
    games = games.filter(g => g.name === args[1]);
    if (games.length > 0) return false; // We already have a game with that name for that user!
    let row = { owner_id: msg.author.id, name: args[1], server_ids:[msg.guild.id], mod_ids: [], settings: settings };
    await cmd.bot.dbManager.insertTableRows(table, [row]);
};
let table = {
    name: 'mafia_game',
    schemaOptions: { owner_id: String, name: String, server_ids: Array, mod_ids: Array, settings: Object }
};
module.exports = {
